# app.py - main streamlit app for the real estate price prediction project
import logging
import os
import sys
...
@st.cache_data
def get_data():
    # load the dataset and cache it so it doesn't reload on every interaction
    return load_data(DATA_PATH)


@st.cache_resource
def get_models():
    # load data, split it, and train both models — cached so it only runs once
    df = get_data()
    X_train, X_test, y_train, y_test, feature_cols = split_data(df)
    lr = train_linear_regression(X_train, y_train)  # train linear regression
    rf = train_random_forest(X_train, y_train)  # train random forest
...
def main():
    ...
    # try to load data and train models; show an error if something goes wrong
    try:
        df = get_data()
    ...
    if page == "Dataset Overview":
        ...
        # show some quick stats at the top using metric cards
        col1, col2, col3 = st.columns(3)
        col1.metric("Total Records", df.shape[0])
        col2.metric("Features", df.shape[1] - 1)  # subtract 1 because price is the target
        ...
        # show descriptive statistics like mean, std, min, max, etc.
        st.subheader("Statistical Summary")
        st.dataframe(df.describe(), use_container_width=True)

        # plot a histogram of property prices to see the distribution
        ...
        # plot a heatmap to see how features are correlated with each other
    ...
    # ---- page 2: model performance comparison ----
        ...
        # evaluate both models on training and testing data using MAE
        ...
        # display the MAE results side by side for each model
        ...
        # put the results in a dataframe so it's easier to plot
        results = pd.DataFrame({
            "Model": ["Linear Regression", "Random Forest"],
            "Train MAE": [lr_train_mae, rf_train_mae],
            "Test MAE": [lr_test_mae, rf_test_mae],
        })
        # create a grouped bar chart comparing train vs test MAE for both models
        ...
        ax.barh(fi["Feature"][::-1], fi["Importance"][::-1], color="teal")  # horizontal bar chart
        ax.set_xlabel("Importance")
        ax.set_title("Feature Importances (Random Forest)")
        st.pyplot(fig)
        plt.close(fig)

    # ---- page 3: predict a price based on user input ----
    elif page == "Predict Price":
        st.header("Predict Property Price")
        st.markdown("Enter the property details below to get a price prediction.")

        # create input fields organized in three columns
        col1, col2, col3 = st.columns(3)
        with col1:
        ...
        # let the user pick which model they want to use for the prediction
        model_choice = st.radio("Select Model", ["Linear Regression", "Random Forest"])

        if st.button("Predict Price", type="primary"):
            # build a dataframe from the user inputs with the same columns the model expects
            ...
            try:
                ...
                prediction = model.predict(input_data)[0]  # get the single predicted price
